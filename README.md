...
Могут использоваться почти везде, в пределах инфраструктурного репозитория. Переменные можно задавать по ходу выполнения play (set_facts, register)

  **Переменные можно задавать в роли, в папке vars:**
          ...
          └── ospf
            ...
            .....
              ...
              │   └── test.yml
              └── vars
              └── main.yml    <----- Задать переменные тут, которые задаются не для хостов ( например в директиве "tags: переменные заданные для группы хостов не видны, поэтому задать их в "./ansible/roles/vars/main.yml" ")
